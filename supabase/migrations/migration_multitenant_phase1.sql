-- Migración: Multi-Tenant "Iron Silo" Architecture
-- Fecha: 17 de Febrero, 2026
-- Objetivo: Aislamiento total de datos por Organización (Tenant)
...
-- Política: Todos pueden VER tenants (necesario para login/registro)
-- En producción estricta, esto se cerraría más, pero para MVP es necesario.
...
-- 4. Actualizar Índices para Performance Multitenant
...
-- Actualizamos las políticas para que SOLO dejen pasar si el tenant_id coincide
...
-- Knowledge Base: EL CAMBIO CRÍTICO
...
CREATE POLICY "Tenant Isolation - Documents"
    ...
    USING (
        (auth.uid() = user_id) -- Propio dueño
        AND
        (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid())) -- Mismo Tenant
    );

-- 6. Función RPC actualizada para Búsqueda (Fase 2 Preparación)
-- Actualizamos la firma pero mantenemos lógica compatible por defecto
