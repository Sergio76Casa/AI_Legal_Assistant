...
function ServiceCard({ title, description, icon, className, colorClass, onClick }: ServiceCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className={cn("group relative p-8 rounded-3xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1 overflow-hidden cursor-pointer", className, colorClass)}
        >
            {/* ... rest of the component body remains same ... */}
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
                <div className="mb-6">
                    <div className="w-12 h-12 rounded-2xl bg-white/60 backdrop-blur-sm flex items-center justify-center mb-4 shadow-sm text-gray-800">
                        {icon}
                    </div>
                ...
                <div className="flex items-center text-sm font-medium text-gray-900 opacity-60 group-hover:opacity-100 transition-opacity">
    ...
    return (
        <section className="px-6 pb-32 max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[400px]">
                {/* Tarjeta Principal - Legal */}
                <ServiceCard
                    title={t('bento.legal_title')}
                    description={t('bento.legal_desc')}
                    icon={<FileText className="w-6 h-6" />}
                    className="md:col-span-2 bg-stone-50 border border-stone-100"
                    colorClass="hover:bg-stone-100"
                    onClick={() => onNavigate('legal-procedures')}
                />

                {/* Tarjeta - Halal/Cultura */}
                <ServiceCard
                ...
                <ServiceCard
                    ...
                    className="bg-stone-50/50 border border-stone-100"
                    colorClass="hover:bg-stone-50"
                ...
                <div
                    ...
                    className="md:col-span-2 relative p-8 rounded-3xl bg-primary text-white overflow-hidden group cursor-pointer"
                >
                    ...
                    <div className="relative z-10 flex flex-col justify-center h-full max-w-lg">
                        ...
                        <p className="text-emerald-100 text-lg font-light mb-8">{t('bento.community_desc')}</p>
                        <button className="self-start px-6 py-3 bg-white text-primary rounded-full font-medium hover:bg-emerald-50 transition-colors flex items-center gap-2">
                            {t('bento.community_btn')}
                            <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full">Telegram</span>
                        </button>
                    </div>
                </div>
            </div>
